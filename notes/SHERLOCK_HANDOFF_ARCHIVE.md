...
=== TASK A — archive Exp C AND Exp C2 raw artifacts via git-LFS (purge-rescue, before ~2026-10-09) ===
Load-bearing artifacts to preserve, for BOTH experiments:
  - the per-position SCORED tables (every number derives from them; they CPU-regenerate the analysis),
  - the backbone PDBs (raw output; Exp C 715, Exp C2 1705 — the divergence/dissolution cases are findings),
...
2. Big artifacts via LFS: git lfs install ; git lfs track "results/*.tar.gz". For EACH of expC, expC2:
     tar -czf results/<exp>_backbones.tar.gz -C $SCRATCH/<exp>/backbones .
     cp $SCRATCH/<exp>/scored_positions.csv results/<exp>_scored_positions.csv   # matches results/*_positions.csv -> LFS
   Manifests (formed/dissolved/nan) are essentially results/expC_gap_perbackbone.csv and
   results/expC2_gap_perbackbone.csv (backbone_id, complex_id, partial_T, irmsd, interface_ok) — copy each
   to results/<exp>_backbone_manifest.csv, adding any nan-coordinate exclusions (Exp C had 18 at T40;
   Exp C2 reported none).
3. POSITIVE CONTROL before pushing: git lfs ls-files MUST list all four big files
   (expC/expC2 × backbones.tar.gz + scored_positions.csv) — else a 100+ MB file would push as a normal
...
GUARDRAIL: GitHub free LFS storage is 1 GB. Rough total here (expC ~200 MB + expC2 ~400 MB) fits with
headroom, but REPORT the running total after push; if it ever nears 1 GB, the backbones are the priority
...
candidate METHOD contribution but crystal-only. Re-run it on the backbones designers use. Note: C2 already
showed the KL DETECTOR (AUROC) generalises; this tests the TRIAGE (capture@k) framing specifically.
